...
Sphere::Sphere(const Vec3& center, math::Real radius, std::shared_ptr<Material> mat, const math::Real reflectFactor, int specularPower) noexcept
    : m_center(center), m_radius(radius), m_radius2(radius * radius), m_material(std::move(mat)), m_color(Vec3(0, 1, 0)), m_reflectFactor(reflectFactor), m_specularPower(specularPower) {}
...
Sphere::Sphere(const Vec3& center, math::Real radius, std::shared_ptr<Material> mat, const Vec3& color, const math::Real reflectFactor, int specularPower) noexcept
    : m_center(center), m_radius(radius), m_radius2(radius * radius), m_material(std::move(mat)), m_color(color), m_reflectFactor(reflectFactor), m_specularPower(specularPower) {}

const Vec3& Sphere::center() const noexcept { return m_center; }
math::Real Sphere::radius() const noexcept { return m_radius; }
const Vec3& Sphere::color() const noexcept { return m_color; }
math::Real Sphere::reflectFactor() const noexcept { return m_reflectFactor; }
int Sphere::specularPower() const noexcept { return m_specularPower; }
...
void Sphere::DrawSphere(Image& image,
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            if (hitSphere && closestHit && closest_t < zbuf[idx]) {
